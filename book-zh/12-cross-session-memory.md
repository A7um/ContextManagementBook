...
技能（`SKILL.md` 文件、`.codex/skills/`、或专门的技能目录）是跨会话记忆的最新形态。其他记忆形式存储事实、偏好或纠正，技能编码的是可复用的**流程**——当匹配的场景出现时，智能体直接照着走。

技能和其他记忆形式的结构性区别：

| 记忆形式 | 存什么 | 示例 |
|---|---|---|
| CLAUDE.md | 不变量 | "始终用 pnpm" |
| 会话记忆 | 会演变的事实 | "认证迁移到了第3轮" |
| Knowledge / Playbooks (Devin) | 成功模式 | "我们怎么部署到 staging" |
| 纠正记录 | 历史错误 | "这里别用 npm" |
| **技能** | **可执行工作流** | "4阶段系统化调试流程" |

`obra/superpowers` 项目是最清晰的例子。它把技能打包成独立的 markdown 文件，每个编码一套完整的方法论：

- **`systematic-debugging`** —— 4阶段根因定位流程：复现 → 隔离 → 假设 → 验证。技能详细规定了每个阶段的具体步骤，包括什么时候用 commit 二分法、什么时候加探针。
- **`test-driven-development`** —— 编码了 RED-GREEN-REFACTOR 循环，带明确规则：先写失败的测试，用最少代码让它通过，再重构。还包括何时写单元测试、何时写集成测试的决策标准。
- **`subagent-driven-development`** —— 编码了完整的分派-审查循环：分解任务 → 派出专业子智能体 → 审查输出 → 整合或重试。

每个技能通常 1000–1500 token——足够紧凑，能和任务上下文一起加载；又足够丰富，替代了智能体每次从零摸索的过程。

**技能跨会话复利增长。**团队一旦写好一个调试技能，以后每个会话都受益于编码好的方法论，不需要逐次学习。第50次调试会话和第5次用的是同一个精炼流程——没有退化，没有漂移。这才是关键的跨会话属性：技能持久化的不只是知识，而是*能力*。

**技能 vs. Devin Playbooks。**两者精神相近——都是把"怎么做 X"编码成持久、可复用的单元。区别在于来源和生命周期：

- Playbooks 是**自动生成**的，从成功的会话中观察有效模式并结晶化。
- 技能是**人工编写**的（或智能体写、人工审查）。资深工程师直接把方法论编码进去。
- 两者都跨所有未来会话持久化，都通过任务匹配触发。

取舍：playbooks 能捕获人类可能说不清楚的隐性经验；技能编码的是观察未必能浮现的显式方法论。最好的系统两者兼用。

**跨组织共享。**Claude Code 和 Cursor 等工具的插件打包层，让技能不仅能跨会话，还能跨组织边界。一家公司写的 `systematic-debugging` 技能可以发布、安装，被另一家公司使用。能跨项目和团队边界的知识单元是全新的类别——之前的跨会话记忆（CLAUDE.md、Devin Knowledge）都局限在单个项目或用户范围内。

从上下文工程角度看，技能是最紧凑的跨会话知识形式，因为它编码的是*流程*而非*事实*。一个 1500 token 的技能，替代了否则需要数十次知识查找、多次纠正检查、加上大量会话内推理才能重建的同一工作流。流程已经预编译好了，智能体直接执行。

## 12.9 ACE 模式——自我更新的上下文

Agentic Context Engineering（ACE）模式把上下文看作**根据智能体表现自我更新的活文档**，而不是静态配置。传统上下文工程问"我们该放什么进上下文？"，ACE 问的是"上下文该如何根据实际效果来演进？"

核心思想：每个会话结束后，评估哪些上下文起了作用、哪些没有，然后相应更新上下文来源。上下文本身成为优化目标，不只是智能体的动作。
...
这是 §12.11 "让学习成为显式操作"原则的延伸：ACE 不只是把纠正写进记忆，还会更新检索规则、技能触发条件，甚至系统提示词本身。记忆不只是在记录发生了什么——它在重塑智能体下次看到什么。

具体更新哪些内容：

- **技能触发条件。**如果一个调试技能被加载了但没使用（任务不需要），降低它对类似未来任务的匹配优先级。如果加载了且被大量使用，强化触发条件。
- **检索配置。**如果智能体反复读取某个记忆文件然后忽略它，这个文件的检索优先级下降。如果读取后立即应用其内容，优先级上升。
- **系统提示词内容。**持续与好结果相关的指令得到强化；与失败或用户纠正相关的指令被修订或移除。
- **技能内容。**当智能体发现了比技能规定更好的方法时，技能本身可以被更新——不只是引用它的记忆。

与 Superpowers 的 `writing-skills` 技能直接相关：智能体可以基于运行中发现的工作流**创建新技能**。如果智能体通过一系列步骤解决了一类新问题且效果很好，ACE 模式把这种隐式发现正式化为持久的、可执行的技能。下一个会话不只是记得这个方法有效——它有现成的流程可以照着走。

关键洞察：传统跨会话记忆问的是"我们学到了什么？"ACE 问的是"下次我们该怎么学得更好？"这是元学习应用于上下文工程——被优化的对象是上下文配置本身。

## 12.10 LangGraph 的生产模式：Checkpointer ≠ Store
