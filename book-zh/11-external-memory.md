...
## 11.8 子智能体输出到文件系统——减少传话游戏的信号衰减

多智能体架构带来一个微妙的上下文工程问题：当子智能体把大块研究结果通过主协调智能体的上下文传递时，信息在每一跳都会丢失或走样。协调器要的是完整分析，子智能体也确实产出了，但结果经过协调器的上下文窗口再传给下一步时，已经被压缩、改写或直接截断了。这跟传话游戏完全一样——每次转述都在衰减信号。

Anthropic 的多智能体研究系统（2025 年 6 月发布）直接解决了这个问题：**子智能体把输出写入文件系统（或外部制品存储），然后只把一个轻量引用传回给协调器。**

这是可恢复压缩（§11.3）在多智能体通信中的应用：

| 子智能体产出内容 | 进入协调器上下文的内容 |
|---|---|
| 8K token 研究报告 | 文件路径 + 3行摘要（约80 token） |
| 生成的代码模块 | 文件路径 + 函数签名（约100 token） |
| 结构化数据提取结果 | 文件路径 + 行数 + schema 摘要（约60 token） |

协调器在引用上操作。当它需要完整内容时——做决策、跨子智能体综合、或把特定细节传给下一个子智能体——读文件就行。其余时间，引用足够支撑路由和编排工作。

这种模式尤其适合结构化输出——代码、报告、数据表。子智能体有专门的提示词，产出的质量高于让通用协调器转述同样内容。一个专门为学术严谨性设计的研究子智能体，比协调器从工具调用里转述出来的要好。写到文件里就保住了全部精度。

实现很直接：子智能体调用工具把成果存入外部系统（文件、数据库、制品存储），然后向协调器返回一个轻量引用对象：
...
协调器看到摘要和路径，就能路由、排优先级、编排后续步骤——不用把 8K token 的研究结果装进自己的窗口。需要完整内容做综合时，一次 `cat` 就拉回来了。

同系统还有一个相关模式：**主研究员在生成子智能体之前，先把研究计划存到记忆里。**理由是防御性的——如果协调器的上下文超过 200K token，会被截断，而研究计划（定义了整个会话方向的东西）可能就此丢失。在上下文膨胀之前把关键编排状态写入文件，就是"在压缩前把关键状态写到文件里"（§10.8）在多智能体层面的应用。计划存活于任何截断事件，因为它在磁盘上，不在窗口里。

两个模式合在一起——子智能体输出到文件系统、协调器状态到文件系统——意味着多智能体通信通过持久存储路由，而非通过易失的上下文窗口。文件系统变成共享内存总线，上下文窗口变成各智能体的工作集。
