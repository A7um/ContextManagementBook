...
## 5.6 技能面临同样的税——以及同样的解法

§5.1 的工具定义税在技能领域有一个精确的平行。假设一个 agent 装了 100 个技能，每个 SKILL.md 正文平均 1,500 token，全部加载就是 150K token——比整个有效窗口还大。经济学完全一样：静态开销，每次推理调用模型都要付，不管它用不用这个技能。

§5.3 的四种生产方案同样适用，只需适配到技能领域：

**1. 渐进式披露（默认做法）。** 会话启动时只加载第一级元数据——技能名称和 YAML frontmatter 里的一行描述，每个约 50 token。这就是技能版本的 Anthropic `defer_loading`。Claude Code 的插件系统在设计上就强制执行这一点：插件技能不被激活就不进上下文。100 个技能时，第一级清单成本约 5K token，完整正文要 150K——基线开销直接降 30 倍。

**2. 语义路由。** 注册表超过约 50 个技能时，仅靠元数据路由就开始变得嘈杂。模型面对一墙的一行描述，选对匹配项越来越难。生产系统使用基于 embedding 的语义路由器：对用户的任务描述做 embedding，跟技能描述的 embedding 比较，加载 top-1 或 top-3 匹配。实测结果：相比全量加载，**token 减少 456 倍**，路由准确率相当。实现很直接——在技能描述上建一个向量索引，每轮开始时查一次，在 LLM 看到任何东西之前。

**3. 层次化命名空间。** Superpowers 风格的插件按工作流阶段给技能分组：头脑风暴 → 规划 → 实现 → 审查。agent 在当前阶段只考虑该阶段的技能——相当于把 logit 掩码的思路用到了技能选择上。这跟 §5.3 里 Manus 的方案结构一致：完整目录始终可用（保留缓存），但活跃子集由当前工作流状态约束。

**4. Frontmatter 作为路由信号。** SKILL.md 文件在 YAML frontmatter 里声明激活条件——`description`、`when`、`globs`、`alwaysApply`。harness 把这些当作低成本路由信号，在决定是否做完整加载之前先判断一轮。`globs: ["**/*.py"]` 的技能只在 agent 处理 Python 文件时激活。`when: "user asks about deployment"` 的技能通过语义匹配任务描述来激活。这些结构化信号在路由时几乎零 token 消耗，防止了不必要的第二级加载。

> **经验法则跟工具一样：** 如果第一级（仅名称 + 描述）的技能注册表就超过了上下文窗口的 15%，你需要路由。低于这个阈值，开销可接受，加载完整清单的简单性胜过引入路由层的复杂性。

技能和工具在结构上的平行不是巧合。两者都是 agent 可以调用的*能力*，区别仅在于它们提供什么——指令还是执行。同样的信息论约束在起作用：模型的注意力有限，上下文窗口里跟当前任务无关的一切都是降低性能的噪音。渐进式披露、语义路由和基于状态的过滤，都是"热路径上能力太多"这个问题的通用解法——不管这些能力是工具 schema 还是技能指令。
