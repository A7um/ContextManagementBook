...
## 4.6 技能作为渐进式上下文：三级加载模式

技能（Skill）——通常以 `SKILL.md` 文件打包——代表了静态上下文层次体系中 2026 年涌现出的新一层。它跟 `CLAUDE.md`（会话启动时始终加载）和 `docs/` 文件（靠 agent 自己判断是否按需加载）都不同。技能采用的是一种**三级渐进式披露**模式，专门为上下文效率而设计。

这种模式之所以有效，是因为技能本质上是*指令*而非参考资料。一个装了 100 个技能的 agent，不可能把它们全部加载——按每个技能正文平均 1,500 token 算，那就是 150K token，比整个有效窗口还大。于是 harness 分三个阶段加载技能，每一级都由需求触发：

### 第一级——仅加载元数据（每个技能约 50 token）

会话启动时，harness 只从 YAML frontmatter 中读取每个技能的名称和一行描述。100 个技能的成本约 5K token——200K 窗口的不到 3%。模型可以扫一眼这份清单，知道有哪些能力可用，而不必为阅读每个技能的完整正文付出代价。

### 第二级——完整 SKILL.md 正文（约 500–2,000 token）

当 agent 判断当前任务匹配某个技能的描述时，加载该技能的完整 SKILL.md 指令。正文包含流程、决策树、约束和规则——执行该技能所需的一切。但关键是，它**不包含**沉重的参考资料。

### 第三级——参考文件（大小不定，按需加载）

厚重的文档、案例、API 规范和 prompt 模板放在技能旁边的 `references/` 子目录里。只有当 agent 在执行过程中真正需要时才加载——比如技能指令引用的 prompt 模板，或生成正确输出所需的 API 规范。

### 生产实例

**Superpowers**（obra/superpowers）：一整套开发方法论被打包成技能。`subagent-driven-development`、`brainstorming`、`test-driven-development` 等技能各自包含一个 SKILL.md 和配套的 prompt 模板文件。agent 只加载当前工作阶段对应的技能。这个项目展示了成熟技能生态的面貌——几十个技能，各自独立，激活条件明确。

**Claude Code 插件**：Anthropic 官方的插件系统把技能打包进命名空间化的包里。发现成本极低（第一级）；完整指令只在激活时加载。官方建议：SKILL.md 控制在 150 行以内，沉重文档移到 `references/`。这个限制的原因是——技能正文跟当前任务一起待在上下文窗口里，一个 500 行的技能正文直接跟 agent 正在理解的代码抢注意力。

**大型注册表的语义路由器**：当技能注册表超过约 50 个技能时，仅靠元数据路由就会变得嘈杂——面对一墙的一行描述，模型很难选对。生产系统使用基于 embedding 的语义路由器：对用户的任务描述做 embedding，与技能描述的 embedding 比较，加载 top-1 或 top-3 匹配。实测结果：相比加载全部技能正文，**token 减少 456 倍**，路由准确率相当。

### 上下文工程的核心洞察

技能形式化了第5章工具搜索模式对工具做的事——只不过目标从*能力*变成了*指令*，渐进式披露的手法完全一致。结构相同：第一级便宜地公告，第二级完整加载，第三级展开参考资料。被渐进式披露覆盖的内容类型在不断扩展：先是工具 schema（§5.3），然后是项目文档（§4.4），现在轮到了技能指令。

> **与 Cursor 的 `.cursor/rules/*.mdc` 的联系**（§4.5）：Cursor 的 glob 触发规则本质上就是通过文件模式触发的第二级技能。技能生态将其泛化，通过显式 YAML frontmatter 控制激活——`description`、`when`、`globs`、`alwaysApply`——给 harness 提供多重路由信号，而不是单一的 glob。

还有一个刻意的设计约束值得一提：插件根目录下的 `CLAUDE.md` **不会**作为项目上下文加载。插件通过技能贡献上下文，不通过 `CLAUDE.md`。这条规则防止了安装插件带来的上下文膨胀——如果没有它，装 20 个插件就会往始终加载层注入 20 个插件级的 `CLAUDE.md`，正是 §4.4 Codex 教训警告过的那种失控增长。

## 4.7 大小规则：500 行以内，通常 300 行以内
