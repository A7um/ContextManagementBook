...
**Token 消耗与性能的相关性。**Anthropic 的多智能体研究评估揭示了一个惊人发现：仅 token 使用量就能解释 80% 的性能方差，工具调用次数和模型选择只是次要因素。这验证了本书的核心论点——token 是基础资源。多智能体系统之所以有效，根本原因是它们通过在隔离的窗口中分配工作来花费更多 token。推论是：衡量智能体性能时，把总 token 消耗（含子智能体 token）作为质量的首要预测指标，而不仅仅当作成本指标。
...
**多智能体系统表现不佳 → token 预算不足。**如果你已经转向多智能体架构但质量没有预期中的提升，检查总 token 消耗。多智能体系统的 token 用量约为单智能体聊天的 15 倍。如果预算限制人为压缩了子智能体深度（更少的工具调用、更短的探索），隔离带来的好处就被不充分的调查抵消了。修复方案：要么增加 token 预算，要么把多智能体限制在高价值任务上——15 倍成本在那些场景下才划算。
...
**评估多智能体架构。**Anthropic 的 Research 系统经验提供了一个模板：关注最终状态评估而非逐轮分析。多智能体系统每次运行的路径都不同——你无法规定一个"正确"的步骤序列。应该评估的是系统是否达到了正确的最终状态。对于研究任务：事实准确性、完整性、来源质量。对于编码任务：测试通过率、代码质量指标、规格符合度。Superpowers 的方法论额外加了显式审查门——先做规格符合审查（是否匹配计划？），再做代码质量审查（是否构建良好？）——这些充当可度量的中间检查点。
...
6. **超过约50次工具调用的任务用子智能体隔离。**单个智能体会话持续超过50次工具调用时，上下文污染几乎是必然的。转向子智能体驱动模式——每个子任务在干净的窗口中执行，只返回摘要——父级上下文体积减少83%（第13章），广度优先任务质量提升最高达90.2%（Anthropic 多智能体研究）。代价：总 token 消耗上升约15倍。收益：无论任务总复杂度如何，父级智能体的注意力质量始终保持高水平。
...
7. **六项改进回报最稳定：** 稳定前缀、工具搜索、压缩、文件化工具输出、结构化历史摘要、子智能体隔离。不知道从哪里下手就从上往下做。

8. **为迭代而设计。** 模块化分层、可配置阈值、采样记录上下文布局、每个变更可回滚。你一定会迭代，把迭代成本降到最低。

9. **度量陷阱：只看顺利路径、笼统聚合、只看平均值、日志不够。** 每一个都在掩盖你最需要修的问题。

10. **Token 消耗预示质量。**Anthropic 的发现——token 使用量解释 80% 的性能方差——重新定义了度量的视角。把总 token 消耗（含子智能体 token）当作质量的先行指标，而不仅仅是成本。质量低又有预算空间时，通过多智能体隔离花更多 token 往往是投入产出比最高的干预手段。
